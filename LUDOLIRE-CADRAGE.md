...
8. **Tranche CE1** — *faite* : table de progression, table des correspondances, décodeur éprouvé, textes, **la banque d'exercices** — 57 exercices couvrant les 42 unités, la dette des dix unités de production soldée —, **le corpus d'images lexicales**, **les images de récit**, **les textes courts** et **les pseudo-mots**. La tranche est complète sur le papier, comme la GS avant elle.
   ...
   **Les exercices à l'écran, et la décision qui manquait pour les rendre** — *fait* : les deux banques portaient **107 exercices écrits**, et l'application n'en affichait **aucun**. Le travail a commencé par refuser la question évidente — « quelle forme pour cet exercice ? » — parce que la réponse était déjà écrite, en prose, dans le champ `geste` de chaque exercice. La lire aurait fait exactement ce que ce projet refuse : une promesse que rien ne vérifie. La décision a donc été rangée dans un fichier, `outils/formes-exercices.json` : pour chacune des **53 mécaniques** des deux banques, la forme d'interaction qui la rend — ou la raison pour laquelle aucune ne la rend. **Deux formes** suffisent : `carte_a_choisir` (lire la question, toucher la carte qui y répond, rendue par le nouvel écran `/exercice`) et `mot_a_trou` (déjà rendue par l'écran du jeu, mais à partir de la **banque du jeu** — la remarque est écrite dans la table plutôt que laissée à deviner). Sur les **95 exercices déclarés jouables**, **11 sont rendus**, **3 sont déclarés** en exception, et les **81** autres sont **comptés avec leur raison**, mécanique par mécanique : l'écran du niveau les affiche sous « Écrit, pas encore jouable ». Un exercice écrit qu'on cache est un mensonge ; un exercice écrit qu'on montre comme injouable est un état.

   La règle du projet a décidé du partage, et le partage est net : **aucun exercice de la GS n'est rendu**, parce que la consigne s'y donne par la voix — *toute consigne se donne par la voix, toute réponse se donne par le geste* — et qu'aucun son n'est enregistré ; un enfant de grande section ne lit pas encore une consigne écrite, et la lui présenter sans elle serait lui demander de deviner. Ce n'est pas une impression : les **19 mécaniques** de la GS sont déclarées non rendues, et **10** d'entre elles nomment cette raison-là — les **neuf autres** dépendent du son autrement, et le disent aussi : l'application doit *produire* le son à localiser, la rime s'entend, la position d'un son dans un mot s'entend, l'exercice suit une voix qui lit. La GS ne manque donc pas d'une forme : elle manque de **son**, et elle le déclare mécanique par mécanique plutôt que sous un motif unique qui les confondrait. Le CE1, lui, lit : **11 exercices** y sont rendus, portant **12 questions**, chacune avec sa bonne réponse, ses intruses et sa phrase-preuve.

   **Ce que la mesure a trouvé, et que personne n'avait décidé.** Trois choses. D'abord `E11` : la banque le déclare jouable, sa mécanique reçoit une forme rendue, et **ses items n'ont jamais été écrits** — son champ `serie` décrit ce qu'il faudrait faire là où les autres portent leurs `questions`. Le contrôle de la banque est vert sur cet exercice : il ne regarde pas la présence des items. Ensuite `E21` et `E55` : leurs trois cartes sont des **titres** de textes courts, et l'écran ne montre **jamais** un titre — c'est délibéré, puisque pour `choisir_titre` le titre du texte *est* la réponse. Les cinq textes courts sont pourtant bien dans le paquet (mesuré, après une première conclusion fausse qu'il a fallu corriger) : ce qui manque n'est pas le matériau mais la forme qui les affiche **avec leur titre**. Ces trois cas ne se rangent pas au même endroit, et la table a dû apprendre à les distinguer : une exception dit désormais **de quel genre elle est** — `champs_manquants` quand il manque à l'exercice ce que sa forme exige, ce qui se **mesure** ; `forme_inadaptee` quand l'exercice porte tout mais que la forme ne sait pas montrer ce qu'il juge, ce qui s'**écrit**. Sans cette distinction, le contrôle refusait une exception exacte comme « périmée » — et une exception refusée à tort est une exception qu'on finit par retirer pour avoir la paix.

   **Le contrôle s'est trouvé lui-même en défaut, et c'est son banc qui l'a dit.** `outils/verifier-formes.py` tient trois choses ensemble — la table, les deux banques, et les écrans réellement présents dans `app/` — et refuse qu'une mécanique soit oubliée, qu'une forme nulle reste sans motif, qu'une exception soit périmée ou mal nommée, ou qu'une forme dise être rendue par un écran qui n'existe pas. Son banc, `outils/tester-formes.py` (**27 cas** : 22 falsifications, un témoin passé par la ligne de commande, une exception nécessaire qui doit être acceptée, et trois comptes), a fait apparaître **deux voies de plantage** : une forme privée de sa clé `exige` faisait lever un `KeyError` au lieu de rapporter le défaut, et une mécanique déclarée sur un niveau inconnu faisait planter le compte sur `banques[niveau]`. Un contrôle qui plante ne rapporte rien — il remplace un message lisible par une trace d'appels, et c'est le pire des deux. Les deux garde-fous sont posés, et le banc a été **falsifié lui-même** : amputé d'une vérification, il cesse de la voir — sans quoi un banc vert ne dirait que sa propre vacuité.

   **Une propriété que personne ne vérifiait : la preuve se retrouve.** L'écran affiche, après une bonne réponse, la phrase du texte qui la justifie — entre guillemets, pour que l'enfant la cherche dans ce qu'il vient de lire. Une paraphrase ne se retrouve pas, et une preuve qu'on ne peut pas retrouver ne prouve rien : elle décore. Mesuré sur les **12 questions** : les 12 phrases-preuves sont **une ligne entière** d'un texte du niveau. Le contrôle du contenu embarqué refuse donc désormais une phrase-preuve introuvable — c'est la vérification que le contrôle des textes annonçait sans pouvoir la faire, et le contrôle du contenu, lui, a les textes et les exercices sous la main. Son banc passe de **23 à 40 cas** (dont 35 falsifications) : bonne réponse comptée parmi les intruses, intruse vide ou répétée, exercice sans question, texte cité qui n'existe pas, mécanique à la fois rendue et déclarée non rendue, exception sans raison.

   **La chaîne compte désormais six contrôles et 76 cas.** `formes:verifier` est le seul contrôle **Python** de `npm run verify` : il est appelé par `python3`, le nom qui existe sur un exécuteur macOS comme sur cette machine, et il tourne dans l'intégration continue comme les autres. Le contenu embarqué porte l'empreinte **`66eedb20c496334b`** (74 171 octets), et il embarque 11 exercices, 12 questions, 39 mécaniques déclarées non rendues et 3 exceptions.

   **La relecture a sa planche** — *fait* : `contenu/planche-ecran-exercice.html` montre les six états de l'écran « S'entraîner » sans rien installer : l'état initial sur iPhone **et** sur Android, la mauvaise réponse, la bonne réponse avec sa phrase de preuve, la fin de séance, et les nouvelles sections de l'écran du niveau. Elle ne propose rien — tout ce qu'elle dessine est livré —, elle dit d'où viennent ses valeurs (`theme.ts`, nommées comme les jetons), et elle avoue ce qu'elle ne montre pas : le défilement, le toucher, et les textes abrégés. Deux détails la rendent honnête plutôt que décorative. D'abord, les **deux teintes qui ne sont pas dans le thème** — le fond vert pâle et le fond rouge pâle des cartes — sont écrites dans `app/exercice.tsx` : la planche les montre telles quelles **et le dit**, parce qu'une valeur de thème qui vit dans un écran est un écart, pas une nuance. Ensuite, l'ordre des cartes est **calculé** par `melange.ts` et la graine de l'écran, jamais recopié : il en sort une information que le relecteur aurait prise pour un défaut — sur la première partie de `E10`, la bonne carte est en **première position**. C'est une permutation possible, la mesure sur les 42 items du jeu donnant 14 / 14 / 14 ; la planche le dit, avec la distribution mesurée sur les 12 questions : **5 / 5 / 2** à la première partie, **3 / 3 / 6** à la seconde — c'est-à-dire que « Recommencer » déplace bien la bonne carte.

   Restent, pour cette tranche, ce qui appartient au porteur du projet : **l'enregistrement** des 33 consignes du CE1 — c'est lui qui rendra jouables les **20 mécaniques du CE1** restées non rendues et, un jour, celles de la GS —, la relecture des exercices **un par un** sur la planche puis sur l'appareil, et les deux décisions pédagogiques que le travail a refusé de prendre à sa place : les items de `E11`, et la forme qui affiche des textes **avec** leurs titres, sans laquelle `E21` et `E55` resteront déclarés.
